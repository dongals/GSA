...
class AvatarModel:
    ...
    def net_set(self, mode):
        ...
        if self.model_parms.train_stage == 2:
            self.pose_encoder = UnetNoCond5DS(
                input_nc=3,
                output_nc=self.net_parms.c_pose,# 64
                nf=self.net_parms.nf,           # 32
                up_mode=self.net_parms.up_mode, # 'upconv'
                use_dropout=False,
            ).cuda()
    ...
    # ?? train.py에서 if stage2에서 stage_load만 하고 stage2_load는 안 함.
    def stage2_load(self, epoch):
        ...
        #여기 추가
        if self.optimizer is not None:
            self.optimizer.load_state_dict(pose_encoder_state["optimizer"])
        if self.scheduler is not None:
            self.scheduler.load_state_dict(pose_encoder_state["scheduler"])
    ...
    def render_free_stage1(self, batch_data, iteration):
        ...
        rendered_images = []
        pose_data = batch_data['pose_data']     # MonoDataset_novel_pose에서 옴. test_pose folder에 있는 것
        ...
        if self.model_parms.smpl_type == 'smplx':
            ...
            live_smpl = self.smpl_model.forward(betas=self.betas, # smpl_parms.pth에서 가져옴
                                global_orient=pose_data[:, :3],
                                transl = transl_data,
                                body_pose=pose_data[:, 3:])
        
        cano2live_jnt_mats = torch.matmul(live_smpl.A, self.inv_mats)
        geom_featmap = self.geo_feature.expand(self.batch_size, -1, -1, -1).contiguous() # net.pth에서 가져옴.
        uv_coord_map = self.uv_coord_map.expand(self.batch_size, -1, -1).contiguous()
    ...
    def render_free_stage2(self, batch_data, iteration):
        ...
        rendered_images = []
        inp_posmap = batch_data['inp_pos_map'] # MonoDataset_novel_pose에서 옴. test_pose folder에 있는 것
        idx = batch_data['pose_idx']

        pose_batch = self.pose(idx)            # MonoDataset_train에서 옴. 
        transl_batch = self.transl(idx)
        # render stage1과 render stage2의 차이: stage1은 novel pose dataset에서 pose,transl을 가져옴.
        pose_data = batch_data['pose_data']
        transl_data = batch_data['transl_data']
        #print('pose_batch shape: ', pose_batch.shape, 'pose_data shape: ', pose_data.shape)
        ...
        if self.model_parms.smpl_type == 'smplx':
            ...
            #                    transl = transl_batch,
            #                    body_pose=pose_batch[:, 3:])
            live_smpl = self.smpl_model.forward(betas=self.betas, # smpl_parms_pred.pth에서 가져오는데 똑같은 거 같음
                                global_orient=pose_data[:, :3],
                                transl = transl_data,
                                body_pose=pose_data[:, 3:])
        ...
        geom_featmap = self.geo_feature.expand(self.batch_size, -1, -1, -1).contiguous() # pose_encoder.pth에서 가져옴

        pose_featmap = self.pose_encoder(inp_posmap)

        #pose_featmap=None으로 바꿔서 test_pose의 inp_posmap이 문제인지 확인 -> stage1에선 live_smpl.A->cano2live_jnt_mats->pt_mats->full_pred로 갔음.
        #만약 pose_encoder를 쓴다면 Unet의 forwarding에서 geo_featmap에 pose_featmap을 더하게 됨.
        ...
        # Here is the condition to check if the image is white
        # 실행결과: Image is completely white만 뜸.
        #image = rendered_images[-1]  # Get the last image generated
        #if torch.max(image) == 1 and torch.min(image) == 1:
        #    print(f"Image {idx} is completely white.")
        #elif torch.mean(image) > 0.95:  # You can also use a threshold close to 1
        #    print(f"Image {idx} is nearly white.")
        #else:
        #    print(f"Image {idx} has some variation.")


        # 여기 return에 points 추가해서 render_novel_pose.py에서 보기
        return torch.stack(rendered_images, dim=0)